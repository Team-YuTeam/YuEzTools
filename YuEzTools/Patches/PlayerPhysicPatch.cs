...
public class ImpNumCheckPatch
{
    public static void Postfix(PlayerPhysics __instance)
    {
        if (GetPlayer.GetImpNums > 3)
        {
            ...
            Error("最大内鬼数比3还大呢！" + AmongUsClient.Instance.GetHost().Character.GetRealName() + "是房主！" + $"{NormalGameOptionsV09.MaxImpostors.Count}个内鬼", "ImpNumCheckPatch");
        }
        ...
        if (GetPlayer.numImpostors > GetPlayer.GetImpNums || GetPlayer.numImpostors > 3)
        {
            ...
            Error("最大内鬼数比预设/3还大呢！" + AmongUsClient.Instance.GetHost().Character.GetRealName() + "是房主！"+ $"{NormalGameOptionsV09.MaxImpostors.Count}个内鬼", "ImpNumCheckPatch");
        }
        
        if (GetPlayer.GetImpNums > 1 && GetPlayer.isHideNSeek)
        {
            SendInGamePatch.SendInGame(GetString("OptHImpMoreThanThree"));
            Error("最大内鬼数比1还大呢！" + AmongUsClient.Instance.GetHost().Character.GetRealName() + "是房主！"+ $"{NormalGameOptionsV09.MaxImpostors.Count}个内鬼", "ImpNumCheckPatch");
        }
