...
public static class VersionShower_Start
{
    public static void Postfix(VersionShower __instance)
    {
        if(ModUpdater.hasUpdate)
            __instance.text.text = TranslationController.Instance.currentLanguage.languageID == SupportedLangs.SChinese || TranslationController.Instance.currentLanguage.languageID == SupportedLangs.TChinese ? $"<color={Main.ModColor}>{Main.ModName}</color> (<color=#DC143C>YuAC有更新!</color>)" : $"<color={Main.ModColor}>{Main.ModName}</color> (<color=#DC143C>YuAC has new version!</color>)";
        else
